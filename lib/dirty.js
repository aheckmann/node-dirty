  ...
  var
    ...
    read = function(gc) {
      self.file.read(16*1024).addCallback(function(chunk) {
        if (!chunk) {
          gc && self.gc();// only auto gc on first load
        ...
        while ((offset = buffer.indexOf("\n")) !== -1) {
          ...
          if (exists && deleted) {
            ...
            // we dont splice _docs b/c that will mess up the refs in _ids
            // after deleting undefined remains but gc() handles that
            delete self._docs[self._ids[doc._id]];          
            delete self._ids[doc._id]; 
                       
          } else if (exists) {
            self._docs[self._ids[doc._id]] = doc;
...
// clean out deleted/undefined docs from this._docs
Dirty.prototype.gc = function() {
  var curDocs = this._docs,
      curIds = this._ids,
      newDocs = [],
      newIds = {},
      doc,
  ...
  for(; i < len; ++i) {
    doc = curDocs[i];  
    if ( doc && doc._id in curIds ) {
      if (doc._id in newIds) {
        newDocs[newIds[doc._id]] = doc;
      } else {
        newIds[doc._id] = (newDocs.push(doc)-1);
      }
    }
  }
...
Dirty.prototype.set = function(id, doc, cb) {
  ...
  if (id in this._ids) { // should overwrite existing docs
